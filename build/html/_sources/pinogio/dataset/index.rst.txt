...
.. tip::
      ...
      **원본 : UTF-8 / x 필드 :  x4326 / y필드 : y4326 / 인코딩(옵션) : UTF-8 / 원본 좌표계(옵션) : 4326 / 대상 좌표계(옵션) : 자유롭게 선택  / 필드 행 번호(옵션) : 0**
...
.. image:: images/datasetSearch.png
    :alt: PINOGIO 데이터셋 - 검색하기

1. 데이터셋 목록 상단의 검색창에 검색할 데이터셋 이름을 입력합니다.
2. 엔터키를 입력하여 검색창에 입력된 데이터셋명을 포함한 데이터셋을 검색합니다.

데이터셋 공유하기
-----------------------------

생성되어있는 데이터셋을 공유하는 기능입니다. 데이터 셋 공유를 통해 다른 사용자가 데이터셋에 접근할 수 있습니다.
다음의 내용은 데이터셋을 공유하는 방법에 대해 설명합니다.

**데이터셋 공유**

.. image:: images/datasetShare.png
    :alt: PINOGIO 데이터셋 - 공유하기

1. 데이터셋 목록 오른쪽의 **공유** 를 클릭합니다.
2. 새로 생성된 데이터셋의 경우 기본적으로 공유하지 않음으로 설정되어 있습니다.
3. **공유링크 만들기** 를 클릭하여 데이터셋 공유를 위한 링크를 생성합니다.

**데이터셋 공유권한 설정**

.. image:: images/datasetShareType.png
    :alt: PINOGIO 데이터셋 - 공유권한 설정

1. 공유링크 만들기가 완료되면 **링크가 있는 모든 웹 사용자에게 공유 - 로그인 필요 없음** 옵션이
기본으로 설정이 됩니다.
2. 공유옵션의 종류와 설명에 대해서는 아래 표 내용을 참고합니다.
3. 아래에 표시된 공유링크를 확인하고, **복사아이콘** 을 클릭하여 클립보드에 복사할 수 있습니다.
4. **끄기 - 공유하지 않기** 옵션이 선택될 경우 **공유링크 제거** 가 활성화 되고 클릭 시 공유를 제거할수 있습니다.
5. **링크가 있는 모든 웹 사용자에게 공유** 또는 **링크가 있는 로그인 사용자 옵션** 이 선택될 경우 공유방법 변경을 클릭할 수 있습니다.
...
생성된 데이터셋의 정보를 조회하는 기능입니다. 데이터 셋의 데이터가 지도에서는 어떻게 표현되는지, 메타데이터 정보는 어떤지, 데이터 필드는 어떻게 이루어져있는지 확인할 수 있습니다.

**데이터셋 정보보기**

데이터셋 목록에서 원하는 데이터셋을 클릭하면 데이터셋의 정보 페이지로 이동할 수 있습니다.

.. image:: images/datasetInfo.png
    :alt: PINOGIO 데이터셋 - 정보보기

**메타데이터 및 필드 정보**

데이터셋 정보 페이지에서 메타데이터를 클릭하면 데이터셋의 메타데이터 정보를, 필드를 클릭하면 데이터셋의 필드 정보를 조회할 수 있습니다.

.. image:: images/datasetMetadataField.png
    :alt: PINOGIO 데이터셋 - 메타데이터 및 필드 정보

**설정 변경하기**

데이터셋 정보 페이지에서 우측 상단의 **톱니바퀴(메타데이터설정)** 를 클릭하여 메타데이터 내용을 수정할 수 있습니다

.. image:: images/datasetSettingModify.png
    :alt: PINOGIO 데이터셋 - 설정 변경하기

**필드 관리하기**

데이터셋 정보 페이지에서 우측 상단의 필드 수정을 클릭하여 메타데이터의 필드를 추가하거나 수정, 삭제할 수 있습니다.

.. image:: images/datasetFieldManage.png
    :alt: PINOGIO 데이터셋 - 필드 관리하기

1. 필드를 수정하려면 수정할 필드 오른쪽의 연필모양아이콘을 클릭합니다.
2. 필드를 삭제하려면 수정아이콘 옆의 삭제아이콘을 클릭합니다.
3. 오른쪽 그림처럼 필드 내용을 수정할 수 있습니다. (필드명과 데이터 타입은 수정이 불가능합니다.)
4. 변경을 클릭하여 수정된 내용을 저장합니다.

새로운 필드를 추가할 수 있습니다. 필드 추가 방법은 새로운 데이터셋 만들기의 비어있는 데이터셋
필드 추가 내용과 같습니다

.. image:: images/datasetFieldAdd.png
    :alt: PINOGIO 데이터셋 - 필드 관리하기

**데이터셋 삭제하기**

생성되어있는 데이터셋이 불필요할 경우에는 데이터셋을 삭제해야 합니다.

.. image:: images/deleteDataset.png
    :alt: PINOGIO 데이터셋 - 삭제하기

1. 데이터셋 정보 페이지에서 우측 상단의 데이터셋 삭제를 클릭합니다.
2. 데이터셋 삭제페이지에서 삭제 확인을 위해 데이터셋 이름을 정확히 입력합니다.
3. 데이터셋 삭제버튼이 활성화되면, 데이터셋 삭제를 클릭하여 데이터셋을 삭제합니다.

**데이터셋 다운로드**

피노지오 데이터셋에 업로드되어있는 데이터를 다운로드 해야 할 때도 있습니다. 피노지오에서는 데이터셋 다운로드 기능을 제공합니다. 데이터셋 정보 페이지 우측 상단의 **다운로드아이콘** 을 클릭하면 데이터셋 파일을 다운로드 받을 수 있습니다. 데이터셋 파일은 Shapefile(zip)파일로 제공됩니다.

.. image:: images/downloadDataset.png
    :alt: PINOGIO 데이터셋 - 다운로드

데이터셋 편집하기
-----------------------------

.. image:: images/editDataset.png
    :alt: PINOGIO 데이터셋 - 편집하기

**주소검색**

주소를 검색하여 지도의 위치를 이동하는 기능입니다. 다음과 같은 과정으로 진행합니다.

1. 오른쪽 상단에 위치한 주소 검색 창에 정확한 도로명 주소를 입력합니다.
2. 입력이 완료되면 엔터키를 눌러 주소를 검색합니다.

.. image:: images/findAddress.png
    :alt: PINOGIO 데이터셋 - 주소검색

**속성정보 변경하기**

선택한 피처(데이터)의 속성정보를 수정하는 기능입니다. 각 속성정보의 타입은 필드의 타입과 동일합니다. 다음과 같은 과정으로 진행합니다.

.. image:: images/attributeEdit.png
    :alt: PINOGIO 데이터셋 - 속성정보 변경하기

1. 속성을 편집할 피처(데이터)를 지도상에서 마우스 왼쪽을 클릭하여 선택합니다.
2. 왼쪽 화면에 나타난 피처(데이터)의 정보를 수정합니다.
3. 상단의 **저장 아이콘(디스켓 아이콘)** 을 클릭하여 저장합니다.

**도형 편집하기**

도형을 편집하는 기능입니다. PINOGIO에서 도형을 편집하는 기능은 데이터의 도형 타입에 따라 달라집니다.  아래는 PINOGIO에서 제공하는 도형 타입별 편집 기능 목록입니다.

.. image:: images/geometryEdit.png
    :alt: PINOGIO 데이터셋 - 도형 편집하기


**1. 포인트 데이터셋 도형 편집하기**

선택한 포인트 타입의 피처(데이터)를 편집합니다. 편집하기 위해서는 지도가 일정 레벨 이상 확대되어야 합니다. 다음과 같은 과정으로 진행합니다.

1. 지도를 오른쪽 상단의 **'편집하려면 확대하세요' 메시지가 없어질 때까지 확대** 합니다.
2. 편집할 피처(데이터)를 마우스 오른쪽을 클릭합니다.
3. 편집할 종류를 클릭하고 편집을 진행합니다.

포인트 데이터셋 도형 편집기능의 종류는 아래와 같습니다.

- 수정 : 피처(데이터)의 도형의 노드 또는 라인을 드래그하여 수정합니다. 수정기능에는 스냅 기능을 지원합니다.
- 삭제 : 피처(데이터)를 삭제합니다.

**2. 라인 데이터셋 도형 편집하기**

선택한 라인 타입의 피처(데이터) 를 편집합니다. 편집하기 위해서는 지도가 일정 레벨 이상 확대되어야 합니다. 다음과 같은 과정으로 진행합니다.

1. 지도를 오른쪽 상단의 **'편집하려면 확대하세요' 메시지가 없어질 때까지 확대** 합니다.
2. 편집할 피처(데이터)를 마우스 오른쪽을 클릭합니다.
3. 편집할 종류를 클릭하고 편집을 진행합니다.

라인 데이터셋 도형 편집기능의 종류는 아래와 같습니다.

- 단순화 : 라인 위 노드의 숫자를 줄여 라인을 단순화 합니다.
- 이동 : 피처(데이터)를 원하는 지점으로 드래그하여 이동합니다.
- 회전 : 피처(데이터)를 드래그하여 회전합니다.
- 짧은축 반전 : 피처(데이터)의 짧은 축을 기준으로 도형을 반전합니다.
- 긴축 반전 : 피처(데이터)의 긴 축을 기준으로 도형을 반전합니다.
- 직선화 : 시작점과 끝을 기준으로 피처(데이터)를 직선화합니다.
- 병합 : 두 개 이상의 연결되어 있는 피처(데이터)를 선택한 후 하나의 라인으로 병합합니다. 이 때 두 개의 피처(데이터)는 끝점이 연결되어 있어야 합니다.
- 수정 : 피처(데이터)의 도형의 노드 또는 라인을 드래그하여 수정합니다. 수정기능에는 스냅 기능을 지원합니다.
- 삭제 : 피처(데이터)를 삭제합니다.
...
폴리곤 데이터셋 도형 편집기능의 종류는 아래와 같습니다.

- 단순화 : 폴리곤의 노드의 숫자를 줄여 폴리곤을 단순화 합니다.
- 이동 : 피처(데이터)를 원하는 지점으로 드래그하여 이동합니다.
- 회전 : 피처(데이터)를 드래그하여 회전합니다.
- 짧은축 반전 : 피처(데이터)의 짧은 축을 기준으로 도형을 반전합니다.
- 긴축 반전 : 피처(데이터)의 긴 축을 기준으로 도형을 반전합니다.
...
피처(데이터)를 편집 및 추가한 후 오른쪽 상단의 **취소아이콘** 을 클릭하여 편집을 취소할 수 있습니다. 편집을 취소하면 저장한 후 또는 처음 상태로 돌아갑니다. 편집을 취소하면 다시 되돌리지 못합니다.

**편집 저장하기**

피처(데이터)를 편집한 후 오른쪽 상단의 저장아이콘을 클릭하여 편집 내용을 저장합니다.

**새로운 도형 추가하기**

오른쪽 상단의 **더하기아이콘** 을 클릭한 후 지도상에 피처(데이터)를를 그려 추가합니다. 그리기 도형의 타입은 데이터셋의 도형 타입에 따라 변경됩니다.
